...
**3. Решение проблем с auto-layout и изоляция регистрации компонентов** *(~1 неделя)*
...
**5. Умная гидрация и 3 режима подгрузки** *(~2-2.5 недели)*
Даем разработчикам контроль над тем, когда и как загружать тяжелый JS-код компонентов. Внедряем три режима:
* `idle`: Стандартный режим (грузим всё сразу, как обычно).
* `on-view`: Компонент «оживает» и подгружает свои зависимости только тогда, когда пользователь доскроллил до него.
* `on-interactive`: Компонент ничего не весит до тех пор, пока пользователь не попытается с ним взаимодействовать (например, навести мышку или кликнуть).
